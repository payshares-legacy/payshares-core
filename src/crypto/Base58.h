...
// Higher-level routines that target the payshares alphabet payshares base58check
...
// Encode a version byte and ByteSlice into Base58Check (with the payshares alphabet).
std::string toBase58Check(Base58CheckVersionByte ver, ByteSlice const& bin);

// Decode a version byte and bytes from Base58Check (with the payshares alphabet).
...
extern const std::string paysharesBase58Alphabet;
}
